...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        #print(len(gameState.getLivingGhosts()))
        for i in range(len(livingGhostPositionDistributions)):
            ghost = max(livingGhostPositionDistributions[i], key=livingGhostPositionDistributions[i].get)
            ghostDict[ghost] = 0
        for ghost in ghostDict:
            ghostDict[ghost] = self.distancer.getDistance(pacmanPosition, ghost)
        #print(ghostDict)
        closeGhost = min(ghostDict, key=ghostDict.get)
        actDict = {}
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            actDict[action] = self.distancer.getDistance(successorPosition, closeGhost)
        return min(actDict, key=actDict.get)
        #print(closeGhost)
