...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #print(positions)
    maxDist = manhattanDistance(positions[0], positions[len(positions)-1]) + 7
    #print(maxDist)
    obsList = []
    ...
    variableDomainsDict[OBS0] = obsList
    variableDomainsDict[OBS1] = obsList
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        print("Net: ", bayesNet)
        #print("Query: ", queryVariables)
        #print("Evidence: ", evidenceDict)
        #print("Order: ", eliminationOrder)
        #print("CPTs: ", bayesNet.getAllCPTsWithEvidence(evidenceDict))
        #print("CPTs: ", bayesNet.getAllCPTsWithEvidence())
        #print(bayesNet.variablesSet())
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #print(currentFactorsList)
        for var in eliminationOrder: #goes through variables to be eliminated
            #print(var)
            #joins factor w/ variable for elimination
            currentFactorsList, fact = joinFactorsByVariable(currentFactorsList, var)
            if len(fact.unconditionedVariables()) != 1: #to skip case of one unconditioned factor
                #print(fact)
                elimFact = eliminate(fact, var) #eliminates variable from factor
                currentFactorsList.append(elimFact) #appends fact w/ eliminated variable
            #print(currentFactorsList)
        fullJoint = joinFactors(currentFactorsList)
        query = normalize(fullJoint)
        return query
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions: 
            probDict[pos] = self.getObservationProb(observation, pacmanPos, pos, jailPos) * self.beliefs[pos]
        #print(probDict)
        #print(prob)
        self.beliefs = DiscreteDistribution(probDict)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        #print(self.getPositionDistribution(gameState, self.getJailPosition()))
        probDict = {}
        for pos in self.allPositions:
            probDict[pos] = 0.0
        for pos in self.allPositions:
            ...
            for newPos in newPosDist:
                probDict[newPos] += newPosDist[newPos] * self.beliefs[pos]
            #print(self.getPositionDistribution(gameState, pos))
        self.beliefs = DiscreteDistribution(probDict)
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        avg = int(self.numParticles/len(self.legalPositions))
